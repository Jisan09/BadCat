...
pawn = ["nsfw","nsfw_gifs","nsfw_gif","60fpsporn","porn","porn_gifs","porninfifteenseconds","CuteModeSlutMode","NSFW_HTML5","the_best_nsfw_gifs","verticalgifs","besthqporngifs","boobs","pussy","jigglefuck","broslikeus","gangbang","passionx","titfuck","HappyEmbarrassedGirls","suicidegirls","porninaminute","SexInFrontOfOthers","tiktoknsfw","tiktokporn","TikThots","NSFWFunny","GWNerdy","WatchItForThePlot","HoldTheMoan","OnOff","TittyDrop","extramile","Exxxtras","adorableporn"]
...
async def bad(event):
    ...
    while(max_try<5):
        ...
        try:
            ...
            return await edit_delete(event,"**(ノಠ益ಠ)ノ  Tou sure this a vaid catagory/subreddit ??**", time=20)
        if "https://i.imgur.com" in media_url and media_url.endswith(".gifv"):
            ...
            try:
                ...
                pass
        try:
            sandy = await event.client.send_file(event.chat_id, media_url, caption=f"<b><a href = {postlink}>{title}</a></b>",reply_to=reply_to,parse_mode="html")
...
async def pussy(event):
    ...
    for m,p,t in zip(media_url,postlink,title):
        ...
        try:
            ...
            if media_url.endswith((".mp4",".gif")):
                await _catutils.unsavegif(event, sandy)
            await edit_or_reply(event,f"**Bluk Download Started.\n\nCatagory :  `{sub_r}`\nFile Downloaded :  {i+1}/{count}**")
...
async def pussy(event):
    ...
    if not intxt and reply:
        intxt = reply.text
    if not intxt:
        return await edit_delete(event, "**ಠ∀ಠ  Reply to valid link or give valid link url as input...you moron!!**")
        ...
        if "https://i.imgur.com" in m and m.endswith(".gifv"):
            ...
            try:
                ...
                try: media_url = links[1]
                    ...
                    media_url = m
        try:
            sandy = await event.client.send_file(event.chat_id, media_url,reply_to=reply_to)
            if media_url.endswith((".mp4",".gif")):
                await _catutils.unsavegif(event, sandy)
            await edit_or_reply(event,f"**Download Started.\n\nFile Downloaded :  {i+1}/{len(plink)}**")
            await asyncio.sleep(2)
        except:
            await event.client.send_message(event.chat_id,f"**Value error!!..Link is :** {m}")
        i+=1
