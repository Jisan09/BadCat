...
async def noods(event):
    ...
    for x in walls:
        ...
        if "We are sorry," not in requests.get(a).text and a not in piclinks:
            ...
            i+=1
        await edit_or_reply(event,f"**📥 Downloaded : {count}/{limit}\n\n❌ Errors : {i}/5**")
        if count == int(limit):
    ...
    try:
        ...
        await event.client.send_file(event.chat_id, piclist,caption = captionlist, reply_to=reply_to_id, force_document=True)
        await event.delete()
